...
export function CharacterTheatre({ projectId, onBack }: { projectId: string; onBack: () => void }) {
  ...
  return <main className="main-content theatre-workspace" id="main-content">
    ...
    {!sessions.isLoading && !sessions.data?.length && <section className="theatre-empty"><h3>还没有试演会话</h3><p>从已确认角色创建一次对话试演，所有重试都会保留为分支。</p><button className="primary-action" type="button" onClick={() => setCreating(true)}>创建第一个剧场会话</button></section>}
